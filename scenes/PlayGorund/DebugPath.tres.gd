...
var simulation = []
export var simulation_steps = 50
export var simulation_delta_t = 0.1
export var simulation_orbit_treshold = 0.3


var width = 1
export var history_lenth = 50
export var gravety_sum = false
export var gravety_components = true

func _draw():
	var now = [ship.translation]
	if(history_lenth>0): _draw_list(past + now, history_color)
	if(gravety_sum): _draw_gravety_sum()
	if(gravety_components): _draw_gravety_components()
	if(simulation_steps>0): _draw_list(simulation,sim_color)
