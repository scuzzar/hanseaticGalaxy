...
@export var max_missions = 3
@export var init_missions = 2
...
func _ready():
	...
	if(cargo!=TYP.CARGO.NONE):
		...
		$GenTimer.start()
	self.mission_generated.connect(Pirates.on_mission_generated)
...
func _generate_mission() -> DeliveryMission:	
	...
	if maxAddition>0 : amount += randi()%int(maxAddition)
	...
	mission.reward = round(single_reward * amount * Globals.RAN.randf_range(1,1.3))
